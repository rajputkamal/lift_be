...
1. Set the values in [`grower-checkout.env.example`](grower-checkout.env.example) in an untracked `.env` or deployment environment. Use only a `rzp_test_` key. The MVP code allows `http://localhost:3000`, `https://green-sprout-store.vercel.app`, and `https://micro-greens.foodieai.in`; an allowed HTTPS origin automatically receives a `SameSite=None; Secure` guest cookie. `ALLOWED_FRONTEND_ORIGINS` can add exact origins without paths or trailing slashes later.
