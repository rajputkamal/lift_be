...
To populate or update the frontend demo catalogue through the actual HTTP APIs, start the backend and run `npm run seed:grower-demo`. The curl-based script finds records by slug, updates existing records, and creates missing records. All demo image fields use Cloudinary URLs. Override the API address with `API_BASE_URL=https://your-api/api/v1 npm run seed:grower-demo`.

Set `CLOUDINARY_URL=cloudinary://API_KEY:API_SECRET@CLOUD_NAME` to enable optional server-side uploads. `POST /api/v1/growers` continues to accept JSON image URLs, and also accepts `multipart/form-data` files in `logo` and `coverImage`. `POST /api/v1/products` accepts either JSON image URLs or up to eight multipart files named `images`. Uploaded files must be JPEG, PNG, WebP, or AVIF and no larger than 5 MB each. Cloudinary credentials remain server-side.
